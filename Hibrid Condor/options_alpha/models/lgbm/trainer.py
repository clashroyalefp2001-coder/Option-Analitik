...
def train_model(
    ...
    """Обучает модель и возвращает словарь со всеми метриками и путями.

    Возвращаемый dict совместим с reports/model_metrics.json — его можно
    напрямую сериализовать.
    """
    ...
    if len(available) < 3:
        raise ValueError(
            f"Слишком мало признаков для обучения: {available}"
        )
    ...
    X_train = train_df[available].astype(float).values
    y_train = train_df[TARGET_COLUMN].values
    X_val = val_df[available].astype(float).values
    ...
    # Регрессионные потери (MSE)
    train_mse = _safe_metric(
        lambda: float(np.mean((pred_train - y_train) ** 2))
    )
    val_mse = (
        _safe_metric(lambda: float(np.mean((pred_val - y_val) ** 2)))
        if len(pred_val) > 0
        else 0.0
    )

    direction = (
        _direction_metrics(y_val, pred_val)
        if len(pred_val) > 0
        else {"precision": 0.0, "recall": 0.0, "f1_score": 0.0, "roc_auc": 0.0}
    )
    ...
    metrics = {
        ...
        "training_accuracy": 0.0,  # для регрессии не используется
