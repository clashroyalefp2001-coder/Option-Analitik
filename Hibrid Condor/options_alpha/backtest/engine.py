...
class OptionBacktestEngine:
    ...
    def run(self, signals: pd.DataFrame, sizes) -> None:
        ...
        # Каждой сделке симулируем траекторию underlying и переоцениваем
        # позицию ежедневно. Для KPI усредняем P&L по симуляциям.
        # Equity curve: складываем дневные P&L всех сделок (паралл.).
        max_days = max(spec["days_to_expiry"] for spec in trade_specs)
        # Дневная сетка
        daily_pnl = np.zeros(max_days + 1, dtype=float)

        for spec in trade_specs:
            sim_results = self._simulate_trade(spec, rng)
            # mean P&L → влияет на equity
            mean_daily_value = sim_results["mean_daily_value"]
            n_days = len(mean_daily_value) - 1  # mean_daily_value[0] = entry value
            # Дневные приращения P&L (mark-to-market) суммируем в equity
            daily_increments = np.diff(mean_daily_value, prepend=mean_daily_value[0])
            for d, inc in enumerate(daily_increments):
                if d <= max_days:
                    daily_pnl[d] += inc
            # Финализируем сделку (для отчёта берём усреднённый итог)
            self.trades.append(sim_results["trade_record"])

        # Equity curve: накапливаем дневные изменения от initial_capital
        equity = np.zeros(max_days + 1, dtype=float)
        equity[0] = self.initial_capital
        for d in range(1, max_days + 1):
            equity[d] = equity[d - 1] + daily_pnl[d]
        self.equity_curve = equity.tolist()
        # Даты — относительные (день 0 = "сегодня")
        today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
        self.equity_dates = [today + timedelta(days=d) for d in range(max_days + 1)]
    ...
    def _simulate_trade(
        ...
        sigma_iv = max(self.sigma_for_pricing, 1e-4)
        dt = 1.0 / 365.0
        mu = self.r - self.dividend  # риск-нейтральная мера для траектории
        ...
        for sim_idx in range(n_sims):
            ...
            opt_value[0] = entry_price  # mark-to-market по входу
            exited = False
            exit_day = n_days
            exit_reason = "expiry"
            ...
            for d in range(1, n_days + 1):
                ...
                S_t = S_path[d]
                if t_remaining <= 0:
                    # День экспирации — внутренняя стоимость
                    if opt_type == "call":
                        opt_value[d] = max(S_t - K, 0.0)
                    else:
                        opt_value[d] = max(K - S_t, 0.0)
                else:
                    # Прямой вызов _crr_price вместо price_american:
                    # в бэктесте греки не нужны, а price_american делает ~6 вызовов CRR
                    # для расчёта греков. Прямой _crr_price — в ~6х быстрее.
                ...
                if not exited:
                    if is_buy:
                        ...
                        # Шорт: цена ВЫРОСЛА → лосс; УПАЛА → профит
                        if sl_level is not None and opt_value[d] >= sl_level:
                            exited = True
                            exit_day = d
                            exit_reason = "stop_loss"
            ...
            # Если сработал SL/TP — позиция закрывается, дальше mark-to-market
            # фиксируется на цене выхода
            if exited:
            ...
            # Daily value: P&L позиции по дням (для equity curve)
            # Для лонга: value_d = (opt_value[d] - exec_entry) * qty - входные издержки
            # После выхода — фиксируем на цене выхода
            value_per_day = np.zeros(n_days + 1, dtype=float)
            value_per_day[0] = -entry_cost  # начальный убыток на издержках входа
            for d in range(1, n_days + 1):
                if d <= exit_day:
                    mtm_price = opt_value[d]
                    mtm_pnl = (mtm_price - exec_entry) * side_sign * qty - entry_cost
                    if d == exit_day:
                        # На дне выхода фиксируем издержки выхода
                        mtm_pnl -= exit_cost
                    value_per_day[d] = mtm_pnl
                else:
                    # После выхода — заморожен P&L на цене выхода
                    value_per_day[d] = pnl
        ...
        trade_record = {
            ...
            "expected_exit_price": exec_entry + side_sign * mean_pnl / max(qty, 1e-9),
            "pnl": mean_pnl,
